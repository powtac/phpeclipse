...
public class LocalDeclaration extends AbstractVariableDeclaration {
	...
	public LocalDeclaration(Expression expr, char[] name, int sourceStart,
			int sourceEnd) {
	...
	public FlowInfo analyseCode(BlockScope currentScope,
		...
		if (flowInfo.isReachable()) {
			bits |= IsLocalDeclarationReachableMASK; // only set if actually
														// reached
		}
		if (initialization == null)
			return flowInfo;

		flowInfo = initialization.analyseCode(currentScope, flowContext,
				flowInfo).unconditionalInits();

		// final int i = (i = 0);
		// no need to complain since (i = 0) part will get the blame
		// if (binding.isFinal() && flowInfo.isPotentiallyAssigned(binding)) {
		// currentScope.problemReporter().duplicateInitializationOfFinalLocal(binding,
		// this);
		// }
	...
	public void checkModifiers() {
		...
		if (((modifiers & AccJustFlag) & ~AccFinal) != 0)
			...
			// AccModifierProblem | AccAlternateModifierProblem -> visibility
			// problem"

			modifiers = (modifiers & ~AccAlternateModifierProblem)
					| AccModifierProblem;
	}

	/**
	 * Code generation for a local declaration: normal assignment to a local
	 * variable + unused variable handling
	 */
	// public void generateCode(BlockScope currentScope, CodeStream codeStream)
	// {
	//
	// // even if not reachable, variable must be added to visible if allocated
	// (28298)
	...
	// // forget initializing unused or final locals set to constant value
	// (final ones are inlined)
	// if (binding.resolvedPosition != -1) { // may need to preserve variable
	// int initPC = codeStream.position;
	// codeStream.generateConstant(inlinedValue,
	// initialization.implicitConversion);
	...
	// // codeStream.lastInitStateIndexWhenRemovingInits = -2; // reinitialize
	// remove index
	// // codeStream.lastInitStateIndexWhenAddingInits = -2; // reinitialize add
	// index
	// }
	...
	// &&
	// (((CastExpression)initialization).innermostCastedExpression().resolvedType
	// == NullBinding)))){
	...
	public void resolve(BlockScope scope) {
		...
		if (tb != null) {
			...
			if (tb.isArrayType()
					&& ((ArrayBinding) tb).leafComponentType == VoidBinding) {
		...
		if ((binding = scope.duplicateName(name)) != null) {
			...
			// the correct constant is harmed if correctly computed at the end
			// of this method
		}

		if (tb == null) {
			if (initialization != null)
				initialization.resolveType(scope); // want to report all
													// possible errors
			return;
		}
		...
		if (initialization != null) {
			if (initialization instanceof ArrayInitializer) {
				TypeBinding initTb = initialization.resolveTypeExpecting(scope,
						tb);
				if (initTb != null) {
				...
				if (initTb != null) {
					if (initialization.isConstantValueOfTypeAssignableToType(
							initTb, tb)
							|| (tb.isBaseType() && BaseTypeBinding.isWidening(
									tb.id, initTb.id))
							|| initTb.isCompatibleWith(tb))
						initialization.implicitWidening(tb, initTb);
					else
						scope.problemReporter().typeMismatchError(initTb, tb,
								this);
				}
			}

			// change the constant in the binding when it is final
			// (the optimization of the constant propagation will be done later
			// on)
			// cast from constant actual type to variable type
			binding.constant = binding.isFinal() ? initialization.constant
					.castTo((tb.id << 4) + initialization.constant.typeID())
					: NotAConstant;
		}
	}
